document.addEventListener('DOMContentLoaded', () => {
  ...
  const checkboxes = [...document.querySelectorAll('.house-card input[type="checkbox"]')];
  const count = document.getElementById('selected-count');
  const updateCount = () => { if (count) count.textContent = checkboxes.filter(item => item.checked).length; };
  checkboxes.forEach(item => item.addEventListener('change', updateCount));
  document.getElementById('select-all')?.addEventListener('click', () => {
    checkboxes.filter(item => !item.closest('.house-card').hidden).forEach(item => item.checked = true);
    updateCount();
  });
